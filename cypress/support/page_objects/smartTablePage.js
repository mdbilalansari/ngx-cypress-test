...
export class SmartTablePage {
  ...
  addNewRecordwithFirstAndLastName(fisrtName, lastName) {
    cy.get('thead').find('.nb-plus').click();
    cy.get('thead')
      .find('tr')
      .eq(2)
      .then((tableRow) => {
        cy.wrap(tableRow).find(' [placeholder="First Name"]').type(fisrtName);
    ...
    cy.get('tbody tr')
      ...
      .find('td')
      .then((tableColumns) => {
        cy.wrap(tableColumns).eq(2).should('contain', fisrtName);
